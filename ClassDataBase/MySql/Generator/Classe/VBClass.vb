...
Namespace MySqlGenerator.ClassGenerator
    Public Class VBClass
#Region "VB.Net Class Fonctions"
        Public Shared Sub Create_VBClassFile(ByVal name As String, ByRef _Path_ScriptFile As TextBox, ByRef ListBox_NameSpace As ListBox, ByVal databasename As String)
            ...
            Dim _PathGenerate_Script As String = Cls_Enumeration.GetPath_MySQL_VbNet_Class(_Path_ScriptFile.Text.Trim, databasename)
            ...
            ds = MySqlHelper.LoadTableStructure_MySql(name)
            ...
            '    Dim Colonne = dt(0).ToString()
            '    Dim TypeCols = dt(1).ToString()
            '    Dim IsPRI_or_MUL = dt(3).ToString() 'MUL / PRI

            '    If IsPRI_or_MUL = "PRI" Then
            '        Id_table = dt(0).ToString()
            '        ListofIndex.Insert(countindex, Colonne)
            ...
            'If ListTAb <= 5 Then
            For Each dt As DataRow In ds.Tables(0).Rows
                If dt(3).ToString = "MUL" Then
                    ListofForeignKey.Add("_" & dt(0).ToString)
                    ...
                    If index.Contains(",") Then
                        If Not listoffound_virguleIndex.Contains(index) Then
                            ...
                            For ind As Integer = 0 To strArr.Length - 1
                                For Each dt_index As DataRow In ds.Tables(0).Rows
            ...
            For Each dt As DataRow In ds.Tables(0).Rows
                If count < cap Then '- 4 Then
                    ...
                    If dt(1).ToString.Contains("(") Then
                        Dim dataType = dt(1).ToString().Split("(")
                        typeData = dataType(0)
                        lengthData = dataType(1).Replace(")", "")
                    Else
                        typeData = dt(1).ToString()
                        lengthData = ""
                    End If

                    cols.Add("_" & Colonne)
                    types.Add(Configuration.ConfigurationSettings.AppSettings(typeData))
                    initialtypes.Add(typeData)
                    length.Add(lengthData)
                    count += 1
                Else
                    Exit For
                End If
            ...
            Try
                For i As Int32 = 1 To cols.Count - 1
                    ...
                    If ListofForeignKey.Contains(cols(i)) Then
                        Dim ForeinKeyPrefix As String = cols(i).Substring(MySqlHelper.ForeinKeyPrefix.Length + 1, cols(i).Length - (MySqlHelper.ForeinKeyPrefix.Length + 1))
            ...
            For i As Int32 = 1 To cols.Count - 1 ' 'On ne cree pas de property pour la derniere column
                ...
                If cols(i) <> "_isdirty" Or cols(i) <> "_LogData" Then
                    If ListofForeignKey.Contains(cols(i)) Then
                        Dim attributUsed As String = cols(i).Substring(MySqlHelper.ForeinKeyPrefix.Length + 1, cols(i).Length - (MySqlHelper.ForeinKeyPrefix.Length + 1))
                        ...
                        Dim ClassName As String = "Cls_" & cols(i).Substring(MySqlHelper.ForeinKeyPrefix.Length + 1, cols(i).Length - (MySqlHelper.ForeinKeyPrefix.Length + 1))
            ...
            objWriter.WriteLine("_id = Convert.ToInt32(MySqlHelper.ExecuteScalar(MySqlHelperParameterCache.BuildConfigDB(), ""SP_Insert_" & nomClasse.Substring(4, nomClasse.Length - 4) & """" & insertstring & IIf(IsAddCreatedBy, ", usr", "") & "))")
            objWriter.WriteLine("Return _id")
            ...
            objWriter.WriteLine("Return MySqlHelper.ExecuteScalar(MySqlHelperParameterCache.BuildConfigDB(), ""SP_Update_" & nomClasse.Substring(4, nomClasse.Length - 4) & """, _id" & updatestring & IIf(IsAddModifBy, ", usr", "") & ")")
            ...
            For i As Int32 = 1 To cols.Count - 2
                ...
                If ListofForeignKey.Contains(cols(i)) Then
                    ...
                    objWriter.WriteLine("" & ForeinKeyPrefix & " = Nothing")
                End If
            Next
            ...
            objWriter.WriteLine("If _idpass <> 0 Then " & Chr(13) _
                            & "Dim ds As DataSet = MySqlHelper.ExecuteDataset(MySqlHelperParameterCache.BuildConfigDB(),""SP_Select_" & nomClasse.Substring(4, nomClasse.Length - 4) & "_ByID"", _idpass)" & Chr(13) & Chr(13) _
            ...
            For i As Int32 = 0 To ListofIndex.Count - 1
                If ListofIndex(i).Contains(",") Then
                    ...
                    objWriter.WriteLine("Dim ds As Data.DataSet = MySqlHelper.ExecuteDataset(MySqlHelperParameterCache.BuildConfigDB(), ""SP_Select_" & nomClasse.Substring(4, nomClasse.Length - 4) & "_" & _strOfIndexToUse & """, " & _strParameterToUse & ")")
                ...
                Else
                    ...
                    objWriter.WriteLine("Dim ds as Dataset = MySqlHelper.ExecuteDataset(MySqlHelperParameterCache.BuildConfigDB(), ""SP_Select_" & nomClasse.Substring(4, nomClasse.Length - 4) & "_" & ListofIndex(i) & """, " & ListofIndex(i) & ")" & Chr(13))
            ...
            objWriter.WriteLine("Public Sub Delete() Implements IGeneral.Delete" & Chr(13) _
                                & "Try" & Chr(13) _
                                & "MySqlHelper.ExecuteNonQuery(MySqlHelperParameterCache.BuildConfigDB(), ""SP_Delete_" & nomClasse.Substring(4, nomClasse.Length - 4) & """, _id)" & Chr(13) & Chr(13) _
            ...
            objWriter.WriteLine("Public Shared Function SearchAll() As List(Of " & nomClasse & ")")
            objWriter.WriteLine("Try ")
            objWriter.WriteLine("Dim objs As New List(Of " & nomClasse & ")")
            objWriter.WriteLine("Dim r As Data.DataRow")
            objWriter.WriteLine("Dim ds As Data.DataSet = MySqlHelper.ExecuteDataset(MySqlHelperParameterCache.BuildConfigDB(), ""SP_ListAll_" & nomClasse.Substring(4, nomClasse.Length - 4) & """)")
            objWriter.WriteLine("For Each r In ds.Tables(0).Rows")
            objWriter.WriteLine("Dim obj As New " & nomClasse)
            objWriter.WriteLine("obj.SetProperties(r)")
            objWriter.WriteLine("objs.Add(obj)")
            objWriter.WriteLine("Next r")
            ...
            For i As Int32 = 1 To cols.Count - 3
                If ListofForeignKey.Contains(cols(i)) Then
                    ...
                    objWriter.WriteLine("Public Shared Function SearchAllBy" & searchtext & "(Byval " & cols(i).ToString.ToLower & " As " & types(i) & ") As List(Of " & nomClasse & ")" & Chr(13) _
                                        ...
                                        & "Dim ds As Data.DataSet = MySqlHelper.ExecuteDataset(MySqlHelperParameterCache.BuildConfigDB(), ""SP_ListAll_" & nomClasse.Substring(4, nomClasse.Length - 4) & cols(i) & """," & cols(i).ToString.ToLower & ")" & Chr(13) _
            ...
            For i As Int32 = 0 To ListofIndex.Count - 1
                ...
                Else
                    ...
                    objWriter.WriteLine("Dim ds As Data.DataSet = MySqlHelper.ExecuteDataset(MySqlHelperParameterCache.BuildConfigDB(), ""SP_Select_" & nomClasse.Substring(4, nomClasse.Length - 4) & "_" & ListofIndex(i) & """, _value)")
                    objWriter.WriteLine(" If ds.Tables(0).Rows.Count < 1 Then")
                    objWriter.WriteLine(" Return False")
                    objWriter.WriteLine("Else")
                    objWriter.WriteLine("If _id = 0 Then")
            ...
            For i As Int32 = 1 To cols.Count - 2
                ...
                If Not columnName.Equals("_CreatedBy") _
                        AndAlso Not columnName.Equals("_DateCreated") _
                        AndAlso Not columnName.Equals("_ModifBy") _
                        AndAlso Not columnName.Equals("_DateModif") Then
